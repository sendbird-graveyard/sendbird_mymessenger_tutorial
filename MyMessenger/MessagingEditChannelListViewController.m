...
- (IBAction)hideMessagingChannel:(id)sender {   
    ...
    if (selectedIndexPath != nil) {
        [SendBird hideMessagingWithChannelUrl:[[channelArray objectAtIndex:selectedIndexPath.row] getUrl]];
        messagingChannelListQuery = [SendBird queryMessagingChannelList];
        [[self delegate] prepareCloseMessagingViewController];
        [self dismissViewControllerAnimated:YES completion:nil];
    }
...
- (IBAction)leaveMessagingChannel:(id)sender {
    NSIndexPath *selectedIndexPath = [self.messagingChannelListTableView indexPathForSelectedRow];
    if (selectedIndexPath != nil) {
        [SendBird endMessagingWithChannelUrl:[[channelArray objectAtIndex:selectedIndexPath.row] getUrl]];
        [[self delegate] prepareCloseMessagingViewController];
        [self dismissViewControllerAnimated:YES completion:nil];
    }
}
